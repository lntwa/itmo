...
import org.example.utilities.LocaleManager;
...
public class GraphicClient extends Application {
     ...
     * Таймер
     */
    private final Timer citiesTimer = new Timer("cities-fetch-thread", true);

    /**
     * Получение организаций
     */
    private final TimerTask cityFetcher = new TimerTask() {
        @Override
        public void run() {
            if (doCityFetch) {
                ...
                if (resp instanceof ResponseWithCities) {
                    ResponseWithCities rwr = (ResponseWithCities) resp;
                    Set<City> newCities = new HashSet<>(Arrays.asList(rwr.getCities()));

                    Platform.runLater(() -> setCities(newCities));
                }
            }
        }
    };
     ...
     */
    private volatile boolean doCityFetch = false;
     ...
     */
    private final ObservableSet<City> cities = FXCollections.observableSet();
     ...
     * Запускает таймер citiesTimer для периодического обновления списка организаций с сервера.
    ...
    public void start(Stage primaryStage) {
        ...
        citiesTimer.schedule(cityFetcher, 0L, SLEEP_TIME);
     ...
     * Если аутентификационные данные равны null, устанавливает флаг doCityFetch в false,
     ...
     * В противном случае устанавливает флаг doCityFetch в true,
     ...
     * Возвращает наблюдаемое множество cities.
     */
    public ObservableSet<City> citiesProperty() {
        return cities;
    }

    /**
     * Метод для получения множества организаций.
     * Преобразует наблюдаемое множество cities в обычное множество и возвращает его.
     */
     ...
     * Сохраняет только те организации, которые уже находятся в наблюдаемом множестве cities,
     * а затем добавляет новые организации из переданной коллекции в наблюдаемое множество cities.
     */
    public void setCities(Collection<City> cities) {
        this.cities.retainAll(cities);

...
public class DBmanager implements CollectionManager {
    ...
    private final ConcurrentLinkedDeque<City> collection = new ConcurrentLinkedDeque<>();
    ...
    synchronized public void loadCollection(ConcurrentLinkedDeque<City> col) {
        try (Connection connection = getConn()) {
            col.clear();
            ...
            while (rs.next()) {
                ...
                City city = new City(cityid, cityName, new Coordinates(x, y), creationDate, area, population, metersabovesealevel, timezone, populationDensity, standardOfLiving, governor);
                col.add(city);
            }

        } catch (SQLException | InvalidFieldException e) {
            col = new ConcurrentLinkedDeque<>();
        }
    }
    ...
    public City getById(int id) {
        try (Connection connection = getConn()) {
            ...
            if (rs.next()) {
                ...
                Integer height = rs.getInt(15);
                Human governor = new Human(nameGovernor, age, height);
                return new City(cityid, cityName, new Coordinates(x, y), creationDate, area, population, metersabovesealevel, timezone, populationDensity, standardOfLiving, governor);
            } else return null;
        } catch (SQLException | InvalidFieldException e) {
            return null;
        }
    ...
    public Integer authenticate(AuthCredentials auth) {
        ...
        // Используем уже существующий метод exists()
        if (exists(auth.getLogin(), auth.getPassword())) {
            return getIdByOwner(auth.getLogin()).intValue();
        }
        return null;
    }
    ...
    @Override
    public int removeIf(Predicate<? super City> predicate) {
        try {
            List<Integer> ids = collection.stream().filter(predicate).map(x -> x.getId()).collect(Collectors.toList());

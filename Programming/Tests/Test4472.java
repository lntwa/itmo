...
public class Test4472 {
    ...
    public void checkAddCommand() {
        assertDoesNotThrow(() -> {
            ...
            java.util.ArrayDeque<Worker> collection = (java.util.ArrayDeque<Worker>) field.get(classWithCollection.newInstance());
    ...
    private static void checkCorrectElementInCollection(Worker element) {
        assertAll(
		() -> assertEquals("dzbzrqmfhy", element.getName()),
		() -> assertEquals(Float.parseFloat("797"), element.getCoordinates().getX()),
		() -> assertEquals(Integer.parseInt("-80"), element.getCoordinates().getY()),
		() -> assertEquals(Integer.parseInt("199"), element.getSalary()),
		() -> assertEquals(Position.MANAGER_OF_CLEANING, element.getPosition()),
 		() -> assertEquals(Double.parseDouble("44"), element.getPerson().getHeight()),
		() -> assertEquals(Double.parseDouble("270"), element.getPerson().getWeight())
        );
    }

    private static void checkUpdateCorrectElementInCollection(Worker element) {
        assertAll(
		() -> assertEquals("dzbzrqmfhyUpd", element.getName()),
		() -> assertEquals(Float.parseFloat("797"), element.getCoordinates().getX()),
		() -> assertEquals(Integer.parseInt("-80"), element.getCoordinates().getY()),
		() -> assertEquals(Integer.parseInt("199"), element.getSalary()),
		() -> assertEquals(Position.MANAGER_OF_CLEANING, element.getPosition()),
    ...
    private static StringBuilder getAddCommandArguments() {
		...
		sb.append("dzbzrqmfhy").append('\n');
		sb.append(Float.parseFloat("797")).append('\n');
		sb.append(Integer.parseInt("-80")).append('\n');
		sb.append(Integer.parseInt("199")).append('\n');
		sb.append("MANAGER_OF_CLEANING").append('\n');
		sb.append(Double.parseDouble("44")).append('\n');
		sb.append(Double.parseDouble("270")).append('\n');
        return sb;
    }
    ...
    private static StringBuilder getUpdateCommandArguments() {
		...
		sb.append("dzbzrqmfhyUpd").append('\n');
		sb.append(Float.parseFloat("797")).append('\n');
		sb.append(Integer.parseInt("-80")).append('\n');
		sb.append(Integer.parseInt("199")).append('\n');
		sb.append("MANAGER_OF_CLEANING").append('\n');
    ...
    private static Stream<Arguments> getFieldsWithTypes() {
        return Stream.of(
				Arguments.of("Worker", "id", "Integer"),
				...
				Arguments.of("Worker", "salary", "int"),
				Arguments.of("Worker", "startDate", "java.util.Date"),
				Arguments.of("Worker", "endDate", "java.time.ZonedDateTime"),
				Arguments.of("Worker", "position", "Position"),
				Arguments.of("Worker", "person", "Person"),
				Arguments.of("Coordinates", "x", "Float"),
				Arguments.of("Coordinates", "y", "int"),
				Arguments.of("Person", "birthday", "java.time.LocalDateTime"),
